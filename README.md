...
| Rocks meeting the cargo ship: a rock that drifts into the carrier is shoved out to its hull surface and set adrift, carried along at the carrier's speed with a 40 u/s nudge outward (the browser used its hull box; the port uses the curved hull profile) | `scripts/cargo_ship.gd` (`bump_rocks`), `scripts/belt.gd` (`place_free`) | ported from depotBumpRocks; the smoke run drops a rock inside the hull and checks it is shoved clear |

## Milestone 16 — the beam's heat, scorches, and the fitting variants

| Piece | Where | Status |
|---|---|---|
| Fix: the sun's shadows were two cascades out to 26 km, and the seam between them (3.1 km out) drew a straight line across any rock face it crossed, with the coarse far cascade self-shadowing slanted faces to black; now one orthographic shadow box as the browser casts (3,300 u ahead, 6,500 near the carrier), texels under a unit | `scripts/lighting.gd` (`update`) | fixed; the smoke run shoots a colossal rock at 4,000 u and the hangar |
| Fix: a rock promoted to LOD 0 vanished (or showed the sky through it) as the ship came close, because its own node sat at the build position while the shader drew it where the rail had carried it, and the renderer culled the node by its mesh bounds; the node now carries bounds that follow the drift | `scripts/belt.gd` (`_lod0_bounds`) | fixed; the smoke run checks a drifted rock |
